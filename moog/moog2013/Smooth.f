...
c     files, and get the synthesis range parameters from the 'dump' file
      rewind nf2out
      rewind nf3out
55    read (nf2out,1002) array
      if (array(1:7).eq.'element' .or.  
     .    array(1:7).eq.'Changin' .or.
     .    array(1:7).eq.'ALL abu' .or.
     .    array(1:7).eq.'Isotopi') go to 55
      if (array(1:7).eq.'MODEL: ') then
...
c*****now read in the raw synthetic spectrum and flip to a flux scale
40    naboff = 80*(nsyn-1)
      nabunds = 0
      nisos = 0
41    read (nf2out,1002,end=2000) array
      if     (array(1:7).eq.'ALL abu' .or. array(1:7).eq.'MODEL: ') then
         go to 41
